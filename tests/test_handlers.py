...
def test_dse_output_selects_completed_trial(
        ...
        for step, value in enumerate(["value1", "value2"], start=1):
            env.runner.experiment_output.update_run(
                ...
                output_models.Run(
                    ...
                    metrics=[output_models.Metric(name="Bandwidth", value=12.5 * step, unit="GB/s")],
                ),
            )
    ...
    stored = output_models.Experiment.model_validate_json((runner.runner.scenario_root / "experiment.json").read_text())
