...
@pytest.fixture
def rest_slurm_system(slurm_system: SlurmSystem) -> SlurmSystem:
    slurm_system.slurm_api = SlurmAPIConfig(
...
def test_slurm_api_request_expands_headers(rest_slurm_system: SlurmSystem, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("SLURM_USER", "cloudai")
    monkeypatch.setenv("SLURM_JWT", "secret")
    response = Mock()
    ...
    request.assert_called_once_with(
        "GET",
        "https://slurm.example.com/slurm/v0.0.38/nodes/",
...
def test_submit_job_through_slurm_api(rest_slurm_system: SlurmSystem, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("CLOUDAI_TEST_SESSION_ENV", "preserved")
    script = """#!/bin/bash
# generated by CloudAI
#SBATCH --job-name=rest-test --output=/shared/stdout.txt
#SBATCH --error=/shared/stderr.txt
#SBATCH --partition=gpu
#SBATCH --account=cloudai
...
srun --container-image=image.sqsh benchmark
"""
    script_path = tmp_path / "job.sbatch"
    script_path.write_text(script)

    with patch.object(SlurmRestClient, "_request", return_value={"job_id": 123}) as request:
    ...
    assert payload["job"] == {
        "name": "rest-test",
        "standard_output": "/shared/stdout.txt",
        "standard_error": "/shared/stderr.txt",
        "partition": "gpu",
        "account": "cloudai",
        "reservation": "nightly",
        "distribution": "block",
        "nodes": [2, 2],
        "required_nodes": "node[01-02]",
...
def test_slurm_api_rejects_unsupported_sbatch_directive(rest_slurm_system: SlurmSystem, tmp_path: Path):
    script_path = tmp_path / "job.sbatch"
    script_path.write_text("#!/bin/bash\n#SBATCH --qos=high\nsrun true\n")

    with (
        patch.object(SlurmRestClient, "_request") as request,
...
def test_slurm_api_rejects_non_sbatch_launcher(rest_slurm_system: SlurmSystem):
    with pytest.raises(JobIdRetrievalError, match="Failed to submit job through Slurm REST API"):
        rest_slurm_system.submit_job("python launcher.py", "launcher")
...
def test_slurm_api_nodes_cancel_and_validation(rest_slurm_system: SlurmSystem):
    ...
    def request(_method: str, service: str, path: str, **_kwargs):
        if service == "slurm" and path == "nodes/":
            return nodes_response
        if service == "slurm" and path == "jobs/":
    ...
    with (
        ...
        assert [(node.name, node.partition, node.state) for node in rest_slurm_system.nodes_from_sinfo()] == [
            ("node01", "main", SlurmNodeState.DRAINED),
            ("node02", "main", SlurmNodeState.ALLOCATED),
            ("node02", "backup", SlurmNodeState.ALLOCATED),
        ]
        assert rest_slurm_system.nodes_from_squeue() == [
            SlurmNode(name="node02", partition="main", state=SlurmNodeState.ALLOCATED, user="cloudai")
        ]
...
def test_slurm_api_validation_requires_git(rest_slurm_system: SlurmSystem):
    with (
        patch("cloudai.systems.slurm.slurm_system.shutil.which", return_value=None),
        pytest.raises(EnvironmentError, match="Required binary 'git' is not installed"),
    ):
        rest_slurm_system.validate_install_environment()
