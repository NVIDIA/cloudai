...
from cloudai.workloads.jax_toolbox import JaxToolboxCmdArgs, JaxToolboxTestDefinition


class TestJaxToolboxSuccessCheck:
    def setup_method(self) -> None:
        self.jax_tdef = JaxToolboxTestDefinition(
            name="n",
            description="d",
            test_template_name="tt",
            cmd_args=JaxToolboxCmdArgs(docker_image_url=""),
        )

    def test_no_profile_stderr_file(self, base_tr: TestRun) -> None:
        result = self.jax_tdef.was_run_successful(base_tr)
        assert not result.is_successful
        assert result.error_message == (
            f"No profile_stderr_*.txt files found in the specified output directory, {base_tr.output_path}. "
    ...
    def test_missing_pax_status_keyword(self, base_tr: TestRun) -> None:
        base_tr.output_path.mkdir(parents=True, exist_ok=True)
        profile_stderr_file = base_tr.output_path / "profile_stderr_1.txt"
    ...
    def test_no_error_files(self, base_tr: TestRun) -> None:
        base_tr.output_path.mkdir(parents=True, exist_ok=True)
        profile_stderr_file = base_tr.output_path / "profile_stderr.txt"
    ...
    def test_cuda_no_device_error_in_profile_stderr(self, base_tr: TestRun) -> None:
        base_tr.output_path.mkdir(parents=True, exist_ok=True)
        profile_stderr_file = base_tr.output_path / "profile_stderr_1.txt"
        profile_stderr_content = "[PAX STATUS]: E2E time: Elapsed time for profiling\n"
    ...
    def test_missing_e2e_time_keyword(self, base_tr: TestRun) -> None:
        base_tr.output_path.mkdir(parents=True, exist_ok=True)
        profile_stderr_file = base_tr.output_path / "profile_stderr_1.txt"
        profile_stderr_content = "[PAX STATUS]: E2E time: Elapsed time for profiling"
        profile_stderr_file.write_text(profile_stderr_content)

        error_file = base_tr.output_path / "error-1.txt"
    ...
    def test_cuda_no_device_error_in_error_file(self, base_tr: TestRun) -> None:
        base_tr.output_path.mkdir(parents=True, exist_ok=True)
        profile_stderr_file = base_tr.output_path / "profile_stderr_1.txt"
        profile_stderr_content = "[PAX STATUS]: E2E time: Elapsed time for profiling"
        profile_stderr_file.write_text(profile_stderr_content)
    ...
    def test_nccl_group_end_error_in_profile_stderr(self, base_tr: TestRun) -> None:
        base_tr.output_path.mkdir(parents=True, exist_ok=True)
        profile_stderr_file = base_tr.output_path / "profile_stderr_1.txt"
        profile_stderr_content = "[PAX STATUS]: E2E time: Elapsed time for profiling\n"
    ...
    def test_nccl_group_end_error_in_error_file(self, base_tr: TestRun) -> None:
        base_tr.output_path.mkdir(parents=True, exist_ok=True)
        profile_stderr_file = base_tr.output_path / "profile_stderr_1.txt"
        profile_stderr_content = "[PAX STATUS]: E2E time: Elapsed time for profiling"
        profile_stderr_file.write_text(profile_stderr_content)
    ...
    def test_heartbeat_error_in_profile_stderr(self, base_tr: TestRun) -> None:
        base_tr.output_path.mkdir(parents=True, exist_ok=True)
        profile_stderr_file = base_tr.output_path / "profile_stderr_1.txt"
        profile_stderr_content = "[PAX STATUS]: E2E time: Elapsed time for profiling\n"
        ...
        assert result.error_message == (
            ...
            f"logs in {base_tr.output_path}. Ensure the servers allocated for this task can reach each other with "
            "their hostnames, and they can open any ports and reach others' ports."
        )

    def test_heartbeat_error_in_error_file(self, base_tr: TestRun) -> None:
        base_tr.output_path.mkdir(parents=True, exist_ok=True)
        profile_stderr_file = base_tr.output_path / "profile_stderr_1.txt"
        profile_stderr_content = "[PAX STATUS]: E2E time: Elapsed time for profiling"
        profile_stderr_file.write_text(profile_stderr_content)
        ...
        assert result.error_message == (
            ...
            f" {base_tr.output_path}. Ensure the servers allocated for this task can reach each other with their "
            "hostnames, and they can open any ports and reach others' ports."
        )

    def test_pyxis_mktemp_error_in_profile_stderr(self, base_tr: TestRun) -> None:
        base_tr.output_path.mkdir(parents=True, exist_ok=True)
        profile_stderr_file = base_tr.output_path / "profile_stderr_1.txt"
        profile_stderr_content = "[PAX STATUS]: E2E time: Elapsed time for profiling\n"
    ...
    def test_pyxis_mktemp_error_in_error_file(self, base_tr: TestRun) -> None:
        base_tr.output_path.mkdir(parents=True, exist_ok=True)
        profile_stderr_file = base_tr.output_path / "profile_stderr_1.txt"
        profile_stderr_content = "[PAX STATUS]: E2E time: Elapsed time for profiling"
        profile_stderr_file.write_text(profile_stderr_content)
