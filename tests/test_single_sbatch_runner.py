...
def test_run_removes_completed_job_from_tracking(sleep_tr: TestRun, slurm_system: SlurmSystem, status: str) -> None:
    ...
    assert runner.jobs == []
    stored = output_models.Experiment.model_validate_json((runner.scenario_root / "experiment.json").read_text())
    ...
    for tr in runner.all_trs:
        runner.experiment_output.update_run(
            ...
            output_models.Run(
                ...
                metrics=[output_models.Metric(name="Bandwidth", value=tr.step * 12.5, unit="GB/s")],
            ),
        )
    with patch.object(
        CloudAIGymEnv,
