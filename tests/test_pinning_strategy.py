...
@pytest.mark.parametrize(
    "cpus,expected",
    [
        (4, ["--cpu-bind=verbose", "--cpus-per-task=4", "--hint=nomultithread"]),
        (12, ["--cpu-bind=verbose", "--cpus-per-task=12", "--hint=nomultithread"]),
        (16, ["--cpu-bind=verbose", "--cpus-per-task=16", "--hint=nomultithread"]),
        (48, ["--cpu-bind=verbose", "--cpus-per-task=48", "--hint=nomultithread"]),
    ],
)
def test_aws_pinning_strategy(cpus: int, expected: list[str]) -> None:
    ps = AwsPinningStrategy(cpus, 1)
    flags = ps.get_pinning_flags()
