...
class KubernetesSystem(System):
    ...
    Attributes
        ...
        output_path (Path): Directory path for output files.
    ...
    def create_job(self, job_spec: Dict[Any, Any], timeout: int = 60, interval: int = 5) -> Tuple[str, str]:
        """
    ...
    def _is_batch_job_observable(self, namespace: str, job_name: str) -> bool:
        ...
        try:
            ...
            if k8s_job:
                logging.debug(f"Batch job '{job_name}' found in namespace '{namespace}' with status: {k8s_job.status}.")
                return True
            else:
                logging.debug(f"Batch job '{job_name}' in namespace '{namespace}' is not yet observable.")
                return False
        except ApiException as e:
            if e.status == 404:
    ...
    def _is_job_running(self, job_namespace: str, job_name: str, job_kind: str) -> bool:
        ...
        if "mpijob" in job_kind.lower():
            return self._is_mpijob_running(job_name, job_namespace)
        elif "job" in job_kind.lower():
            return self._is_batch_job_running(job_name, job_namespace)
        else:
            error_message = (
    ...
    def _is_mpijob_running(self, job_name: str, job_namespace: str) -> bool:
        """
    ...
    def _is_batch_job_running(self, job_name: str, job_namespace: str) -> bool:
        """
        ...
        """
        try:
            k8s_job = self.batch_v1.read_namespaced_job_status(name=job_name, namespace=job_namespace)
            conditions = k8s_job.status.conditions if k8s_job.status and k8s_job.status.conditions else []
            ...
            # If the job has been created but is neither complete nor failed, it is considered running
            if any(condition.type == "Created" and condition.status == "True" for condition in conditions):
                return True
            ...
            if e.status == 404:
                logging.debug(
                    f"Batch job '{job_name}' not found in namespace '{job_namespace}'. It may have completed and been "
                    "removed from the system."
                )
                return False
            else:
    ...
    def delete_job(self, namespace: str, job_name: str, job_kind: str) -> None:
        ...
        Args:
            ...
            job_kind (str): The kind of the job ('MPIJob' or 'Job').
        """
        logging.debug(f"Deleting job '{job_name}' of kind '{job_kind}' in namespace '{namespace}'")
    ...
    def store_pod_logs(self, namespace: str, pod_name: str, log_file_path: Path) -> None:
        """
        ...
        Args:
            ...
            log_file_path (Path): The file path where the logs will be stored.
