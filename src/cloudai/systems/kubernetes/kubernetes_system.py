...
from .kubernetes_node import KubernetesNode, KubernetesNodeState


class KubernetesSystem(System):
    """
    Represents a Kubernetes system, encapsulating the system's configuration.
    ...
    Attributes
        ...
        partitions (Dict[str, List[KubernetesNode]]): Mapping of partition names to lists of KubernetesNodes.
        ...
        cache_docker_images_locally (bool): Whether to cache Docker images locally for the Kubernetes system.
        groups (Dict[str, Dict[str, List[KubernetesNode]]]): Nested mapping where the key is the partition name and the
            value is another dictionary with group names as keys and lists of KubernetesNodes as values, representing the
    ...
    def format_node_list(cls, node_names: List[str]) -> str:
        ...
        Mimicking the compact display found in systems like Kubernetes's sinfo command output.
    ...
    def __init__(
        ...
        partitions: Dict[str, List[KubernetesNode]],
        ...
        groups: Optional[Dict[str, Dict[str, List[KubernetesNode]]]] = None,
        ...
        Args:
            ...
            partitions (Dict[str, List[KubernetesNode]]): Partitions in the system.
            account (Optional[str]): Account name for charging resources used by this job.
            distribution (Optional[str]): Specifies alternate distribution methods for remote processes.
            mpi (Optional[str]): Indicates the Process Management Interface (PMI) implementation to be used for
                inter-process communication.
            ...
            ntasks_per_node (Optional[int]): Specifies the number of tasks that can run concurrently on a single node.
            cache_docker_images_locally (bool): Whether to cache Docker images locally for the Kubernetes system.
            groups (Optional[Dict[str, Dict[str, List[KubernetesNode]]]]): Nested mapping of group names to lists of
                KubernetesNodes within partitions, defining the group composition within each partition. Defaults to an
    ...
    def get_partition_nodes(self, partition_name: str) -> List[KubernetesNode]:
        """
        Return a list of KubernetesNode objects in the specified partition.

        Args:
            partition_name (str): The name of the partition.

        Returns:
            List[KubernetesNode]: Nodes belonging to the specified partition.
    ...
    def get_group_nodes(self, partition_name: str, group_name: str) -> List[KubernetesNode]:
        """
        Return a list of KubernetesNode objects in the specified group within a partition.
        ...
        Returns:
            List[KubernetesNode]: Nodes belonging to the specified group within the partition.
    ...
    def get_available_nodes_from_group(
        ...
        Returns:
            List[KubernetesNode]: Objects that are potentially available for use.
        ...
        for node in self.groups[partition_name][group_name]:
            if node.state in grouped_nodes:
                ...
                if node.state == KubernetesNodeState.ALLOCATED and node.user == current_user:
                    continue
                if node.state in grouped_nodes:
    ...
    def is_node_in_system(self, node_name: str) -> bool:
        """
        Check if a given node is part of the Kubernetes system.
    ...
    def is_job_running(self, job_id: int, retry_threshold: int = 3) -> bool:
        """
        Determine if a specified Kubernetes job is currently running by checking its presence and state in the job queue.

        This method queries the Kubernetes job queue using 'squeue' to identify if the
        ...
        while retry_count < retry_threshold:
            ...
            if "Socket timed out" in stderr or "kubernetes_load_jobs error" in stderr:
                retry_count += 1
                logging.warning(
                    f"An error occurred while querying the job status. Retrying... ({retry_count}/{retry_threshold}). "
                    "CloudAI uses Kubernetes commands by default to check the job status. The Kubernetes daemon can become "
                    ...
                    "for this warning. Please ensure that the Kubernetes daemon is running and responsive."
                )
                continue
    ...
    def is_job_completed(self, job_id: int, retry_threshold: int = 3) -> bool:
        """
        Check if a Kubernetes job is completed by querying its status.
    ...
    def scancel(self, job_id: int) -> None:
        """
        Terminates a specified Kubernetes job by sending a cancellation command.
    ...
    def update_node_states(self) -> None:
        """
        Update the states of nodes in the Kubernetes system.
        ...
        the state of nodes and the users, and updates the corresponding KubernetesNode instances in the system.
        """
    ...
    def convert_state_to_enum(self, state_str: str) -> KubernetesNodeState:
        """
        Convert a Kubernetes node state string to its corresponding enum member.
        ...
        Args:
            state_str (str): State string from Kubernetes, could be full name, abbreviated code, or with a "*" suffix.

        Returns:
            KubernetesNodeState: Corresponding enum member, or UNKNOWN_STATE for unmatched states, NOT_RESPONDING for "*"
                suffix.
        ...
        state_abbreviations = {
            "alloc": KubernetesNodeState.ALLOCATED,
            "comp": KubernetesNodeState.COMPLETING,
            "down": KubernetesNodeState.DOWN,
            "drain": KubernetesNodeState.DRAINED,
            "drng": KubernetesNodeState.DRAINING,
            "fail": KubernetesNodeState.FAIL,
            "failg": KubernetesNodeState.FAILING,
            "futr": KubernetesNodeState.FUTURE,
            "idle": KubernetesNodeState.IDLE,
            "maint": KubernetesNodeState.MAINTENANCE,
            "mix": KubernetesNodeState.MIXED_ALLOCATION,
            "npc": KubernetesNodeState.USING_NETWORK_PERFORMANCE_COUNTERS,
            "plnd": KubernetesNodeState.PLANNED_STATE,
            "pow_dn": KubernetesNodeState.PENDING_POWER_DOWN_STATE,
            "pow_up": KubernetesNodeState.POWERING_UP_STATE,
            "resv": KubernetesNodeState.RESERVED,
            "unk": KubernetesNodeState.UNKNOWN_STATE,
        }

        core_state = state_str.split()[0].upper().rstrip("*+~#!%$@^-")

        if state_str.endswith("*"):
            return KubernetesNodeState.NOT_RESPONDING

        try:
            return KubernetesNodeState(core_state)
        except ValueError:
            ...
            if abbrev in state_abbreviations:
                ...
                return KubernetesNodeState.UNKNOWN_STATE
