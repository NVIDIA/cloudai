...
class KubernetesSystem(System):
    ...
    def _run_genai_perf(self, job: KubernetesJob) -> None:
        ...
        logging.debug(f"Executing command to make wrapper script executable in pod={frontend_pod} cmd={kubectl_exec_cmd}")
        try:
            result = subprocess.run(kubectl_exec_cmd, capture_output=True, text=True, timeout=60 * 10)
            logging.debug(f"chmod exited with code {result.returncode}. stdout: {result.stdout}, stderr: {result.stderr}")
