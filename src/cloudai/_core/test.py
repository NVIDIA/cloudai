...
class Test:
    ...
    def gen_job_spec(self, output_path: Path, job_name: Optional[str] = None) -> Any:
        """
        Prepare the job specification using the associated test template.

        Args:
            output_path (Path): Path to the output directory.
            job_name (Optional[str]): The name of the job (optional).
            **kwargs: Additional arguments that may be required by the test template's gen_job_spec method.

        Returns:
            Any: The job specification as generated by the test template.
        """
        ...
        return self.test_template.gen_job_spec(
            ...
            extra_cmd_args=self.extra_cmd_args,
