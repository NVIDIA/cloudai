...
class MegatronBridgeSlurmCommandGenStrategy(SlurmCommandGenStrategy):
    ...
    def _build_launcher_parts(  # noqa: C901
        ...
        add_field("gpus_per_node", "-gn", args.gpus_per_node or self.system.gpus_per_node)
        ...
        gpus_per_node = args.gpus_per_node or self.system.gpus_per_node
        if gpus_per_node and self.system.supports_gpu_directives:
            additional_slurm_params.append(f"gpus-per-node={gpus_per_node}")
            additional_slurm_params.append(f"gres=gpu:{gpus_per_node}")
