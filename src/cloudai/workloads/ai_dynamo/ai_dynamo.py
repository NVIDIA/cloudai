...
class CustomWorkload(Workload):
    """Generic workload script."""

    model_config = ConfigDict(extra="allow")

    name: str = "custom_workload"
    cmd: str = "hostname"
    script: File = File(Path(__file__).parent.parent / "ai_dynamo/custom_workload.sh")
...
class AIDynamoCmdArgs(CmdArgs):
    ...
    custom_workload: CustomWorkload = Field(default_factory=CustomWorkload)
...
class AIDynamoTestDefinition(TestDefinition):
    ...
    workloads: str = "genai_perf.sh,lmbench.sh,custom_workload.sh"
    ...
    def validate_test_definition(self) -> "AIDynamoTestDefinition":
        ...
        if self.cmd_args.custom_workload.repo:
            self.git_repos.append(self.cmd_args.custom_workload.repo)

        # Validate benchmark names
        workloads = self.workloads.split(",")
        for workload in workloads:
            if workload not in [
                self.cmd_args.genai_perf.script.src.name,
                self.cmd_args.lmbench.script.src.name,
                self.cmd_args.custom_workload.script.src.name,
            ]:
    ...
    def installables(self) -> list[Installable]:
        ...
        result = [
            ...
            self.cmd_args.custom_workload.script,
