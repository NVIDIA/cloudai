...
class AIDynamoTestDefinition(TestDefinition):
    ...
    def hugging_face_home_path(self) -> Path:
        raw = self.extra_env_vars.get("HUGGING_FACE_HOME")
        if not isinstance(raw, str) or not raw.strip():
            raise ValueError("HUGGING_FACE_HOME must be set and non-empty")
        path = Path(raw)
        if not path.is_dir():
            raise FileNotFoundError(f"HUGGING_FACE_HOME path not found at {path}")
        return path
