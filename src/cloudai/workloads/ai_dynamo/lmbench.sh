...
cmdline_args="${cmdline_args} --output ${result_dir}/lmbench_bench_output.csv ${key} ${extra_args}"
# launch lmbench
...
for qps in ${all_qps//,/ }; do
    ...
    eval "$cmd $cmdline_args --qps $qps --output $output_file" 2>&1 > ${artifacts_dir}/lmbench_bench_output_${qps}.log
